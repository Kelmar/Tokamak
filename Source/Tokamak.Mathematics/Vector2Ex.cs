...
namespace Tokamak.Mathematics
{
    ...
    public static class Vector2Ex
    {
        extension(in Vector2 vector)
        {
            ...
            public static bool TryFromArray(in ReadOnlyMemory<float> array, out Vector2 result)
            ...
            /// <param name="result">The resulting vector from the array, or <seealso cref="Vector2.Zero"/> if a vector could not be created.</param>
            /// <returns>True if the array had enough parameters to create a vector, false if not.</returns>
            public static bool TryFromArray(in ReadOnlySpan<float> array, out Vector2 result)
            {
                if (array.Length < 2)
                {
                    result = Vector2.Zero;
            ...
            /// <summary>
            /// Convert a enumeration of floats to a Vector2.
            /// </summary>
            /// <remarks>
            ...
            /// </remarks>
            public static Vector2 FromEnumeration(IEnumerable<float> floats)
            {
                var array = floats.ToArray();
                return ToVector2(array);
            }
        ...
        /// <summary>
        /// Convert ReadOnlyMemory of floats to a Vector2.
        /// </summary>
        /// <remarks>
        /// If the array is not of sufficient length, then the vector will be initialized with zeros in the left over places.
        /// 
        ...
        /// </remarks>
        public static Vector2 ToVector2(this in ReadOnlyMemory<float> a)
            => ToVector2(a.Span);

        /// <summary>
        /// Convert ReadOnlySpan of floats to a Vector2.
        /// </summary>
        /// <remarks>
        /// If the array is not of sufficient length, then the vector will be initialized with zeros in the left over places.
        /// 
        /// If you wish to validate that the array is sufficient length during conversion use the <seealso cref="TryFromArray(float[], out Vector2)"/>
        /// </remarks>
        public static Vector2 ToVector2(this in ReadOnlySpan<float> a)
            => new Vector2(a.Length > 0 ? a[0] : 0, a.Length > 1 ? a[1] : 0);
    }
}
